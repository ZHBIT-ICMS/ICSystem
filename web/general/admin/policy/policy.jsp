...
<head>
    ...
    <script type="text/javascript" charset="utf-8">
                ...
                var p = parent.dj.dialog({
                    ...
                    buttons : [ {
                        ...
                        handler : function() {
                            var f = p.find('form');
                            f.form({
                                url : '${pageContext.request.contextPath}/pubPolicy!edit.action',
                                success : function(d) {
                    ...
                    onLoad : function() {
                        ...
                        var editor = f.find('textarea[name=content]').xheditor({
                            ...
                            html5Upload : true,
                            upMultiple : 4,
                            upLinkUrl : '${pageContext.request.contextPath}/pubPolicy!upload.action',
                            upLinkExt : 'zip,rar,txt,doc,docx,xls,xlsx',
                            upImgUrl : '${pageContext.request.contextPath}/pubPolicy!upload.action',
            ...
            var p = parent.dj.dialog({
                ...
                buttons : [ {
                    ...
                    handler : function() {
                        var f = p.find('form');
                        f.form({
                            url : '${pageContext.request.contextPath}/pubPolicy!add.action',
                            success : function(d) {
                                var json = $.parseJSON(d);
                                if (json.success) {
                                    datagrid.datagrid('reload');
            ...
            if (rows.length > 0) {
                parent.dj.messagerConfirm('请确认', '您要删除当前所选项目？', function(r) {
                    if (r) {
                        ...
                        $.ajax({
                            url : '${pageContext.request.contextPath}/pubPolicy!delete.action',
                            data : {
                                ids : ids.join(',')
                            },
                            dataType : 'json',
        ...
        function showCdesc(rowIndex) {
            ...
            var p = parent.dj.dialog({
                title : '政策法规标题[' + row.title + ']',
                modal : true,
                maximizable : true,
                width : 800,
                height : 600,
                content : '<iframe src="${pageContext.request.contextPath}/pubPolicy!showContent.action?id=' + row.id + '" frameborder="0" style="border:0;width:100%;height:99.4%;"></iframe>'
            });
